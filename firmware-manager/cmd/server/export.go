...
Examples:
  ...
  firmware_manager export --kinds FirmwareImage --output ./firmwareimage-backup
...
func runExport(ctx context.Context, format, output string, kinds []string, perType bool) error {
	...
	if len(kinds) > 0 {
		...
		resourceKinds = []string{"FirmwareImage", "FirmwareUpdateJob"}
	}
...
func exportResourceKind(ctx context.Context, kind, output, format string, perType bool) (int, error) {
	...
	switch kind {
	case "FirmwareImage":
		items, e := storage.Queryfirmwareimages(ctx).All(ctx)
		if e != nil {
			return 0, fmt.Errorf("failed to query firmwareimages: %w", e)
		}
